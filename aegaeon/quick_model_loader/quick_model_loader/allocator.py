...
class Allocator:
    ...
    def __init__(self, size: int, device: Union[str, Device]) -> None:
        if size <= 0:
            raise MemoryError(
        ...
        if isinstance(device, str):
            self.device = Device.from_str(device)
        elif isinstance(device, Device):
            self.device = device
        else:
            raise TypeError(
                "The device should either be a str or Device enum when initialize an Allocator"
            )
...
class CUDAllocator(Allocator):
    def __init__(self, size: int, device_id: Optional[int] = None) -> None:
        super().__init__(size, device=Device(DeviceType.CUDA, device_id))


class CPUAllocator(Allocator):
    def __init__(self, size: int) -> None:
        super().__init__(size, device=Device(DeviceType.CPU))


class PinnedAllocator(Allocator):
    def __init__(self, size: int) -> None:
        super().__init__(size, device=Device(DeviceType.PINNED))
